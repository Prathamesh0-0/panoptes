...
Interactive encrypt/decrypt and sign/verify endpoints for the PQC Status page.
"""
import uuid
import json
...
from backend.crypto.pqc_vault import (
    ...
    get_pqc_vault_status, PQC_AVAILABLE, PQC_KEM_ALGORITHM
)
from backend.crypto.audit_signer import (
    sign_entry, verify_entry, get_pqc_sig_status
)
...
async def pqc_status():
    """Combined PQC status for the dashboard panel."""
    vault_status = get_pqc_vault_status()
    sig_status = get_pqc_sig_status()
    return {
        "opa_running": is_opa_running(),
        "opa_mode": "Real OPA (Rego)" if is_opa_running() else "Inline Python fallback",
        "vault": vault_status,
        "signing": sig_status,
        "overall_pqc_real": vault_status["pqc_real"] and sig_status["pqc_real"],
    }
...
async def sign_message(req: SignRequest):
    ...
    return {
        ...
        "message": "Entry signed with post-quantum signature.",
    }


@router.post("/verify")
async def verify_message(req: VerifyRequest):
    result = verify_entry(req.entry)
    return result
